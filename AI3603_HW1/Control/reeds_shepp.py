...
def calc_all_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=STEP_SIZE):
    ...
    for path in paths:
        ...
        # convert global coordinate    转换成全局坐标
...
def set_path(paths, lengths, ctypes):
    ...
    assert path.L >= 0.01     # 检测 是否大于0.01
    paths.append(path)
...
def generate_path(q0, q1, maxc):    # q0 起始位置  q1 终止位置
    dx = q1[0] - q0[0]
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2]
    c = math.cos(q0[2])             # 位置姿态统一化
    s = math.sin(q0[2])             # 可以 用旋转矩阵 优化
    x = (c * dx + s * dy) * maxc
    y = (-s * dx + c * dy) * maxc
...
def calc_curvature(x, y, yaw, directions):
    ...
    for i in range(1, len(x) - 1):
        ...
        if np.isnan(curvature):  # np.isnan 判断是否为空值
            curvature = 0.0
